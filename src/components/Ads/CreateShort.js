...
function CreateAds() {
  ...
  const [minPoints] = useState(10); // Minimum points allowed for an ad
  ...
  const handleCreateAd = async () => {
    ...
    if (isNaN(pointsToSpend) || pointsToSpend < minPoints || pointsToSpend % 10 !== 0) {
      setMessage(`Please enter a valid amount of points (minimum ${minPoints} and multiples of 10).`);
      return;
    }
